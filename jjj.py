...
class Switch:
    ...
    def __init__(self, position, delay=0):  
        """Cчитывается """
...
class ButtonWire:
    ...
    def __init__(self, gpio_pin_btn, led1, led2, delay=0):
    ...
    def btn_switch_condition(self):
        while True:
            if GPIO.input(self.__gpio_pin_btn) == GPIO.LOW:
                GPIO.output(self.__led1, GPIO.LOW)
                time.sleep(self.__delay)
                GPIO.output(self.__led2, GPIO.HIGH)
            else:
                GPIO.output(self.__led2, GPIO.LOW)
                time.sleep(self.__delay)
                GPIO.output(self.__led1, GPIO.HIGH)
...
if __name__ == "__main__":
    ...
    btReal = br.btn_switch_condition()
